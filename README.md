...
- **R scripts** (`.R`) and **R Markdown files** (`.Rmd`) used to
  generate figures and tables in the paper and to build the additional
  reports. These use routines and data in the R package `RanCh` and
  simulations run in C but are otherwise self contained.
...
- The **LaTeX source** files for the paper, `population.tex` and
  bibliography.bib.
...
- A `domain_tex/` sub-folder containing descriptions of choice domains
  used in the experiment. (This material is used not only in the paper
  but also in the additional reports.)
...
- A `documents/` sub-folder containing documents related to the
  experiment, including
  - `Certificate_of_approval.pdf`, a certificate of ethics approval for
    Tony Marley from the University of Victoria.
  - `Consent_form.pdf` and `Consent_form.docx`, the text of the consent
    agreement approved by participants in the experiment
  - `Recruitment_Compensation_procedures.pdf`, a description of
    recruitment and compensation procedures used by SSI
  - `ssi-2017-panel-book.pdf`, SSI’s Global panel book for 2017, the
    year of the experiment.
- A `screenshots/` sub-folder containing sample screenshots for each
  domain, as seen by participants in the experiment
- A `design/` sub-folder with miscellaneous documents related to the
  experimental design.
- A `figures/` sub-folder with graphics used in the experiment,
  including R code for reproducing some of the figures.

### `documents/`

This folder contains

- For each choice domain, a document describing the domain, a sample
  screenshot from the domain, a table of count and proportions data from
  the experiment, and a data visualization device for displaying choice
  proportions from menus of size two and three.
- A document containing a complete list of comments from the
  participants in the experiment

### `R/`

- `make_figures.R` creates figures `binary_BF.pdf` and `multiple_BF.pdf`
  using dma_tables.RData simulation summary data in `data/`
