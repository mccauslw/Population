...
for (domain in 0:n_domains) {
	if (domain == 0) {
		t <- read.table(here('data', 'population_priorsim'))
	}
	else {
		# Process posterior information for given domain
		table_name <- here('data', sprintf('population_postsim_%i', domain))
	...
	for (model in 0:n_models) {
		if (model==0) {
			w <- rep(1, dim(t)[1])
			mcresult <- list(se = 0.0, est = 1)
		}
		else {
			w <- exp(t[, sprintf('w%i', model)])
			# Returns est, se (estimated mean, std error)
			mcresult <- mcse(w, method = "obm")
		}
...
save(dma_array, dma_table, dm_array, dm_table,
     file = here("data", "dma_tables.RData"))
