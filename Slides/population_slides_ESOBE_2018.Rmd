...
date: "ESOBE 2018, October 11, 2018"
...
## Unknown choice probabilities

Let $T \equiv \{\mathit{Red},\mathit{Purple},\mathit{Pink}\}$.
The unknowns are four probability vectors:
...
Here is a special case of a two-parameter ($\alpha$, $\lambda$) prior we use:

- The four probability vectors are mutually independent ($\lambda=0$)
- Binary probabilities are all $\mathrm{Be}(\frac{\alpha}{2}, \frac{\alpha}{2})$.
- Ternary probability is $\mathrm{Di}(\frac{\alpha}{3}, \frac{\alpha}{3}, \frac{\alpha}{3})$.

We will take $\alpha = 2$ in the following examples.

## Four 95\% High Posterior Density (HPD) regions
...
## Bringing theory to bear I: Stochastic Transitivity

```{r ST, echo=FALSE, message=FALSE, warning=FALSE}
op = par(mfrow=c(1, 2), mar = c(4, 4, 0.5, 0.5))
tripl = triplot(label=c('Red', 'Purple', 'Pink'))
tripoints(0.9, 0.1, 0.0, pch=binary.pch)
tripoints(0.0, 0.9, 0.1, pch=binary.pch)
tripoints(0.1, 0.0, 0.9, pch=binary.pch)
tripl = triplot(label=c('Red', 'Purple', 'Pink'))
tripoints(0.9, 0.1, 0.0, pch=binary.pch)
tripoints(0.0, 0.9, 0.1, pch=binary.pch)
tripoints(0.9, 0.0, 0.1, pch=binary.pch)
```

## Bringing theory to bear II: Regularity
...
## Bringing theory to bear III: Random utility/preference

Let $T$ be the universe of objects

This set of conditions (the Block-Marschak conditions) is necessary and sufficient for random utility:
...
* Two different priors with the same fours marginals:
...
![Asymmetric Dominance Effect](./Figures/RCM_SBIES_figure1.pdf)

```{r asymdom, echo=FALSE, fig.height=3, fig.width=3, fig.align='center'}
x = c(0.2, 0.25, 0.75)
y = c(0.7, 0.75, 0.25)
plot(x, y,
  xlab="Level of attribute 1", ylab="Level of attribute 2",
  xlim=c(0, 1), ylim=c(0, 1), pch=20, bty="l", xaxt='n', yaxt='n', mar=c(1,1,1,0))
...
## Typical asymmetric dominance pattern
...
1. Analysis using data on individual choice (paper)
1. Analysis using data on population choice (this presentation)
